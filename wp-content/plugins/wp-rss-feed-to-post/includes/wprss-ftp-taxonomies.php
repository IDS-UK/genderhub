...
function wprss_ftp_no_terms_msg( $taxonomy ) {
	return "<span class='ftp-tax-no-terms'>The <strong>{$taxonomy}</strong> taxonomy has no available terms.</span>";
}
		...
		<td>
			<?php
				if ( count($post_type_taxonomies) === 0 ) :
					...
					echo "<p id='ftp-no-taxonomies'>The Post Type <strong>{$all_post_types[$post_type]}</strong>  has no taxonomies available!</p>";
				else :
					...
					<p>
						...
						?>
						<label for="ftp-auto-terms-<?php echo $id; ?>">Auto create terms from the feed items for this taxonomy</label>

						<span class="ftp-tax-row-buttons">
							<button title="Refresh the terms, if they've changed" type="button" class="ftp-tax-section-refresh button-secondary" data-id="<?php echo $id; ?>">
								<fa class="fa fa-fw fa-refresh"></fa>
								<span>Refresh terms</span>
							</button>
					
							<button title="Remove this entry" type="button" class="ftp-tax-section-remove button-secondary" data-id="<?php echo $id; ?>">
...
function wprss_ftp_add_taxonomies_to_post( $post_id, $feed_id, $sp_item ) {
	...
	// If it matches, add the settings taxonomies to the meta taxonmies
	...
	foreach( $meta as $entry ) {
		...
		if ( $auto === TRUE ) {
			...
			// Repeat the taxonomy slug in an array with the same length as the terms array
			$taxonomies_array = array_fill( 0, count($feed_terms), $taxonomy );
