...
#=== META BOXES REGISTRATION =============================================================================
	...
	public function add_meta_boxes() {
		...
		add_meta_box(
			...
			__( 'Feed to Post - Taxonomies', 'wprss' ),			// $title 
	...
	public function get_meta_fields( $what = '', $source_id = null ) {
		$fields = array(
			...
			'tax'	=>	array(
				...
				'post_taxonomy' 	=> array(
					'label'			=>	'Post Taxonomy',
					'desc'			=>	'Choose the taxonomy to apply to imported feeds.',
					'type'			=>	'msg',
					'text'			=>	'Please wait ...',
				...
				'post_terms' => array(
					'label'		=>	'Post Terms',
					'desc'		=>	'Choose the taxonomy terms to apply to imported feeds.',
					'type'		=>	'msg',
					'text'		=>	'Please wait ...',
					'ignore'	=>	TRUE
				),
				'post_tags' => array(
					'label'			=>	'Post Tags',
					'desc'			=>	'Enter the post tags, comma separated, to assign to posts imported from this source.',
					'type'			=>	'text',
			...
			'images'	=>	array(
				...
				'image_min_width' => array(
					'label'			=>	'Image minimum width',
					'desc'			=>	'Choose the minimum width for image imports.',
					'type'			=>	'number',
					'placeholder'	=>	'Ignore',
				...
				'image_min_height' => array(
					'label'			=>	'Image minimum height',
					'desc'			=>	'Choose the minimum height for image imports.',
					'type'			=>	'number',
					'placeholder'	=>	'Ignore',
					'ignore'		=>	true
				),
				...
				'featured_image' => array(
					'label'		=>	'Featured Image to use',
					'type'		=>	'dropdown',
					'options'	=>	array(
						'first'		=>	'First image in post content',
...
#== META BOX RENDERERS =================================================================================
	...
	public function render_taxonomies( $post ) {
		...
		<tr id="wprss-ftp-taxonomies-add-section" class="wprss-tr-hr">
			<th>
				...
				<?php if ( class_exists( 'WPRSS_Help' ) ) echo WPRSS_Help::get_instance()->do_tooltip( WPRSS_FTP_HELP_PREFIX.'taxonomies' ); ?>
			</th>
			<td></td>
		</tr>
	...
	public function render_image_min_dimensions( $post ) {
		...
		<tr>
			...
			<td>
				<i class="fa fa-fw fa-arrows-h" title="Image Width"></i>
				<input class="wprss-number-roller" type="number" id="<?php echo $width_name; ?>"  name="<?php echo $width_name; ?>" min="0" placeholder="Width" value="<?php echo $width ;?>" />
				<i class="fa fa-fw fa-times"></i>
				<input class="wprss-number-roller" type="number" id="<?php echo $height_name; ?>" name="<?php echo $height_name; ?>" min="0" placeholder="Height" value="<?php echo $height ;?>" />
				<i class="fa fa-fw fa-arrows-v" title="Image Height"></i>
				<?php
					if ( class_exists( 'WPRSS_Help' ) ) {
						echo WPRSS_Help::get_instance()->do_tooltip( WPRSS_FTP_HELP_PREFIX.'image_min_dimensions' );
					}
				?>				
			</td>
		</tr>
	...
	public function render_extraction_rules( $post, $field_id, $field, $meta ) {
		...
		$input_field = '<input type="text" name="' . self::META_PREFIX . 'extraction_rules[]" value="{{value}}" placeholder="CSS Selector" /> ';
...
#== META DATA SAVING ===================================================================================
	...
	public function save_post_meta( $post_id, $post ) {
		...
		if ( $post_id && ! empty( $_POST[self::META_PREFIX.'post_taxonomy'] ) ) {
			...
			$terms = $_POST[self::META_PREFIX.'post_terms'];
			$autos = $_POST[self::META_PREFIX.'auto_terms'];
			$final = array();
			for( $i = 0; $i < count($taxonomies); $i++ ) {
				$final[$i] = array();
				$final[$i]['taxonomy'] = $taxonomies[$i];
				$final[$i]['terms'] = $terms[$i];
				$final[$i]['auto'] = $autos[$i];
			}
			update_post_meta( $post_id, self::META_PREFIX.'taxonomies', $final );
		}
	}
