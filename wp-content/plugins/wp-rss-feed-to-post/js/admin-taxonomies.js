(function($){
	...
	$(document).ready( function(){
		...
		$('#ftp-add-taxonomy').click( function(){
			...
			$(this).html('<i class="fa fa-fw fa-refresh fa-spin"></i> Please Wait..').prop('disabled', true);
		...
		post_type_selector.change( function(){
			...
			add_button.html('<i class="fa fa-fw fa-refresh fa-spin"></i> Please Wait..').prop('disabled', true);
			$.post(
				ajaxurl,
				{
	...
	function initChosens( tr ) {
		...
		term_selectors.chosen({
			no_results_text: 'No terms found that match',
			placeholder_text_multiple: 'Choose terms'
		});
		
		var taxonomy_selectors = tr ? tr.find('.ftp-taxonomy') : $('.ftp-taxonomy');
		// Set the texts
		taxonomy_selectors.chosen({
			no_results_text: 'No taxonomy was found that matched',
			placeholder_text_single: 'Choose a taxonomy'
		})
