...
;(function ( $, window, document, undefined ) {
	...
	var pluginName = "ubermenu",
		defaults = {
			...
			retractors: true,
			touchOffClose: true,
			moveThreshold: 20,				//Distance until tap is cancelled in deference to move/scroll
	...
	Plugin.prototype = {
		...
		init: function () {
			...
			if( this.settings.debug ) console.log( 'Initializing UberMenu' );
...
*/
		...
		initializeSubmenuToggleTouchEvents: function (){
			...
			this.$ubermenu.on( 'click' , '.ubermenu-target' , function(e){ plugin.handleClicks( e , this , plugin ); } );
		...
		reenableSubmenuToggleMouseEvents: function( plugin ){
			...
			if( plugin.settings.debug ) console.log( 'reenableSubmenuToggleMouseEvents' );
		...
		initializeRetractors: function() {
			...
			//If this device supports touch interactions, set up the retractors
			if( this.touchenabled ){
				this.$ubermenu.on( this.touchStart , '.ubermenu-retractor' , function(e){ plugin.handleSubmenuRetractorStart( e , this , plugin ); } );	// > .ubermenu-target
			}
			//Otherwise, remove the mobile retractors
			else{
		...
		initializeTouchoffClose: function(){
			...
			$( document ).on( 'mouseup.ubermenu_clickoff' , function(e){ plugin.handleTouchoffClose( e, this , plugin ); } ); //use mouseup instead of click for firefox
		...
		// change by nyasro //
		var n = this.$ubermenu.find( '.ubermenu-current-menu-item' ).first().parents( '.ubermenu-item' ); if(!n.hasClass('ny_hide')){n.addClass( 'ubermenu-current-menu-ancestor' );}
		 // end of change by nyasro //
		 },
		...
		handleTouchInteraction: function( e , target , plugin ){
			...
			if( plugin.settings.debug ) console.log( 'touchstart ' + $(this).find( '> a' ).text() , e );
		...
		preventInteractionOnScroll: function( e , target , plugin ){
			...
			if( plugin.settings.debug ) console.log( 'touchmove', e );
			...
			else if( plugin.settings.debug ) console.log( 'no touch points found!' );
		...
		handleTap: function( e , target , plugin ){
			...
			if( plugin.settings.debug ) console.log( 'handleTap [' + $target.text() + ']', e.type );
...
console.log( 'scrolltarget = ' + scrolltarget );
			if( scrolltarget ){
				...
				if( $target_el.size() > 0 ){
					$( 'html,body' ).animate({
						...
						function(){
							plugin.closeSubmenu( $link.parent( '.menu-item' ) );
							//after scroll
						});
			...
			//Handle clicks (where default was already prevented)
				...
				if( follow && e.isDefaultPrevented() ){
					console.log( 'default prevented, follow link' );
		...
		handleMouseClick: function( e , target , plugin ){
			if( plugin.settings.debug ) console.log( 'handleMouseClick', e );
		...
		handleMouseover: function( e , target , plugin ){
			...
			if( plugin.settings.debug ) console.log( 'handleMouseenter, add mouseleave', e );
		...
		handleMouseout: function( e , li , plugin ){
			...
			if( plugin.settings.debug ) console.log( 'handleMouseout, remove mouseout', e );
		...
		handleSubmenuRetractorStart: function( e , li , plugin ){
			...
			if( plugin.settings.debug ) console.log( 'handleSubmenuRetractorStart ' + $( li ).text());
		...
		handleSubmenuRetractorEnd: function( e , li , plugin ){
			...
			if( plugin.settings.debug ) console.log( 'handleSubmenuRetractorEnd ' + $parent_item.find('> .ubermenu-target').text());
		...
		handleResponsiveToggle: function( e , toggle , plugin ){
			...
			if( plugin.settings.debug ) console.log( 'handleResponsiveToggle ' , e );
			...
			if( !$li.hasClass( 'ubermenu-active' ) ){
				...
				if( plugin.transitions ){
					...
					$li.find( '> .ubermenu-submenu' ).on( plugin.transitionend + '_opensubmenu', function(){
						if( plugin.settings.debug ) console.log( 'finished submenu open transition' );
		...
		closeSubmenu: function( $li , tag , plugin ){
			...
			if( plugin.settings.debug ) console.log( 'closeSubmenu ' + $li.find( '>a' ).text() + ' [' + tag + ']' );
		...
		resetHandlers: function( $target ){
			if( this.settings.debug ) console.log( 'reset handlers on ' + $target.text() );
