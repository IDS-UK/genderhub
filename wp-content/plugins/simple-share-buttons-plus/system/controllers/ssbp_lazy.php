...
defined('ABSPATH') or die('No direct access permitted');
    ...
    function ssbp_lazy_callback()
    {
        ...
        // the buttons!
        echo $ssbpButtons->get_ssbp_buttons($_POST['ssbpurl'], $_POST['ssbptitle'], $_POST['ssbpsharetext'], $_POST['ssbpshorturl'], $_POST['ssbppostid']);
...
class ssbpShareButtons
{
    ...
    public function ssbpShareButtons($url = null)
    {
        // get ssbp settings
    ...
    public function get_ssbp_buttons($urlCurrentPage, $strPageTitle, $strShareText, $urlShortened = '', $post_id = null)
    {
        // variables
        ...
        if (in_array($this->ssbp_settings['text_placement'], array('above', 'left')) && $strShareText != '') {
            // add share text
            $this->htmlShareButtons .= '<span class="ssbp-share-text">'.stripslashes_deep($strShareText).'</span>';
        ...
        if (in_array($this->ssbp_settings['text_placement'], array('right', 'below')) && $strShareText != '') {
            // if above add line break
    ...
    public function get_ssbp_share_counts()
    {
        ...
        // if some share counts were found
        if ($counts) {
            $this->ssbpShareCountData = json_decode($counts, true);
        }
        // no share counts were found in the db
        else {
            // an array of networks with share counts
            $hasCounts = array(
                'facebook', 'google', 'linkedin', 'pinterest', 'reddit', 'stumbleupon', 'twitter', 'vk',
            );
    ...
    public function ssbp_email()
    {
        ...
        // email share link
        $this->htmlShareButtons .= '<a href="mailto:?Subject='.$this->strPageTitle.'&amp;Body='.$this->ssbp_settings['email_message'].'%20'.($this->urlShortened == '' ? $this->urlCurrentPage : $this->urlShortened).'" class="ssbp-btn ssbp-email" data-ssbp-title="'.$this->strPageTitle.'" data-ssbp-url="'.$this->urlCurrentPage.'" data-ssbp-site="Email" '.($this->ssbp_settings['ga_onclick'] == 'Y' ? 'onclick="ssbpTrackGA(\''.$this->urlCurrentPage.'\', \'Email\'); return false;"' : null).'>';
    ...
    {
        // facebook share link
        $this->htmlShareButtons .= '<a href="http://www.facebook.com/sharer.php?u='.$this->urlCurrentPage.'" class="ssbp-btn ssbp-facebook" '.($this->ssbp_settings['rel_nofollow'] == 'Y' ? ' rel="nofollow"' : null).' data-ssbp-title="'.$this->strPageTitle.'" data-ssbp-url="'.$this->urlCurrentPage.'" data-ssbp-site="Facebook" '.($this->ssbp_settings['ga_onclick'] == 'Y' ? 'onclick="ssbpTrackGA(\''.$this->urlCurrentPage.'\', \'Facebook\'); return false;"' : null).'>';

        // not using custom images
        if ($this->ssbp_settings['custom_images'] != 'Y') {
            $this->htmlShareButtons .= '<span class="ssbp-text">Facebook</span>';
        } else {// using custom images
            $this->htmlShareButtons .= '<img src="'.$this->ssbp_settings['custom_facebook'].'" title="Share this on Facebook" class="ssbp" alt="Share this on Facebook" />';
        }
        ...
        if ($this->ssbp_settings['counters_enabled'] == 'Y') {
            ...
            if ((int) $this->ssbpShareCountData['facebook'] >= intval($this->ssbp_settings['min_shares'])) {
                $this->htmlShareButtons .= '<span class="ssbp-total-facebook-shares ssbp-each-share">'.ssbp_format_number($this->ssbpShareCountData['facebook']).'</span>';
            }
        }
    }
    ...
    {
        // if using ssbp api
        if ($this->ssbp_settings['share_api'] == 'Y') {
            ...
            $htmlFacebookShareDetails = wp_remote_get('http://graph.facebook.com/'.$this->urlCurrentPage, array('timeout' => $this->ssbp_settings['count_timeout']));
    ...
    public function ssbp_pinterest()
    {
        ...
        if ($this->ssbp_settings['pinterest_use_featured'] || $this->ssbp_settings['pinterest_use_ssbp_meta'] == 'Y') {
            ...
            $this->htmlShareButtons .= '<a href="http://pinterest.com/pin/create/bookmarklet/?is_video=false&url='.$this->urlCurrentPage.'&media='.$strSSBPImage.'&description='.$strSSBPDescription.'" class="ssbp-btn ssbp-pinterest" data-ssbp-title="'.$this->strPageTitle.'" data-ssbp-url="'.$this->urlCurrentPage.'" data-ssbp-site="Pinterest_force" '.($this->ssbp_settings['ga_onclick'] == 'Y' ? 'onclick="ssbpTrackGA(\''.$this->urlCurrentPage.'\', \'Pinterest\'); return false;"' : null).'>';
        }
    ...
    public function ssbp_tumblr()
    {
        ...
        $url =  str_replace('http://', '', $this->urlCurrentPage);
        $url =  str_replace('https://', '', $url);

        // tumblr share link
        $this->htmlShareButtons .= '<a href="http://www.tumblr.com/share/link?url='.$url.'&amp;name='.$this->strPageTitle.'" class="ssbp-btn ssbp-tumblr" '.($this->ssbp_settings['rel_nofollow'] == 'Y' ? 'rel="nofollow"' : null).' data-ssbp-title="'.$this->strPageTitle.'" data-ssbp-url="'.$url.'" data-ssbp-site="Tumblr" '.($this->ssbp_settings['ga_onclick'] == 'Y' ? 'onclick="ssbpTrackGA(\''.$this->urlCurrentPage.'\', \'Tumblr\'); return false;"' : null).'>';
    ...
    public function ssbp_twitter()
    {
        ...
        // twitter share link
        $this->htmlShareButtons .= '<a href="https://twitter.com/share?url='.($this->urlShortened == '' ? $this->urlCurrentPage : $this->urlShortened).'&amp;text='.$twitterShareText.'&amp;hashtags='.$this->ssbp_settings['twitter_tags'].$ssbpTwitterVia.'" class="ssbp-btn ssbp-twitter"'.($this->ssbp_settings['rel_nofollow'] == 'Y' ? 'rel="nofollow"' : null).' data-ssbp-title="'.$this->strPageTitle.'" data-ssbp-url="'.$this->urlCurrentPage.'" data-ssbp-site="Twitter" '.($this->ssbp_settings['ga_onclick'] == 'Y' ? 'onclick="ssbpTrackGA(\''.$this->urlCurrentPage.'\', \'Twitter\'); return false;"' : null).'>';

        // not using custom images
        if ($this->ssbp_settings['custom_images'] != 'Y') {
            $this->htmlShareButtons .= '<span class="ssbp-text">Twitter</span>';
        } else {// using custom images
            $this->htmlShareButtons .= '<img src="'.$this->ssbp_settings['custom_twitter'].'" title="Tweet about this" class="ssbp" alt="Tweet about this" />';
        }
        ...
        if ($this->ssbp_settings['counters_enabled'] == 'Y') {
            ...
            if ((int) $this->ssbpShareCountData['twitter'] >= intval($this->ssbp_settings['min_shares'])) {
                $this->htmlShareButtons .= '<span class="ssbp-total-twitter-shares ssbp-each-share">'.ssbp_format_number($this->ssbpShareCountData['twitter']).'</span>';
            }
        }
    }
    ...
    public function ssbp_twitter_count()
    {
        ...
        $htmlTwitterShareDetails = wp_remote_get('http://urls.api.twitter.com/1/urls/count.json?url='.$this->urlCurrentPage, array('timeout' => $this->ssbp_settings['count_timeout']));
