					  ...
					  <div class="container-fluid">
					    ...
					    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
					      <ul class="nav navbar-nav navbar-right">
					        <li><a href="https://simplesharebuttons.com/plus/tutorials/" target="_blank">Tutorials</a></li>
                            ...
                            if ($status === false || $status != 'valid') {
                                ...
                                $htmlHeader .= '<li><button type="button" class="ssbp-btn-inactive btn-danger ssbp-float-right-btn btn btn-primary" data-toggle="modal" data-target="#ssbpLicenseModal">License inactive</button></li>';
                            }
    $htmlHeader .= '</ul>
					    </div>
					  </div>
...
function ssbp_admin_dashboard()
{
    ...
    $htmlShareButtonsDash .= '<div class="row">';
        ...
        $htmlShareButtonsDash .= '<p>It is strongly recommended that you copy and paste the support details below to ensure that assistance can be provided and a solution found as swiftly as possible. The more information provided the better. It is also recommended that you export your SSBP settings and provide the .txt file too.</p>';
            ...
            $htmlShareButtonsDash .= '<p><a href="https://simplesharebuttons.com/forums/forum/simple-share-buttons-plus" target="_blank"><button class="btn btn-primary btn-block">Visit the Support Forum</button></a></p>';
...
function ssbp_dashboard_stats()
{
    ...
    $htmlDashboard .= '<th>Date</th>';
        ...
        foreach ($arrLatestShares as $latestShare) {
            ...
            $htmlDashboard .= '<tr>';
            $htmlDashboard .= '<td style="padding-top:5px;"><a href="'.$latestShare['url'].'">'.$latestShare['title'].'</a></td>';
            $htmlDashboard .= '<td>'.$latestShare['site'].'</td>';
    ...
    echo '</thead>';
            ...
            echo '<tbody>';
                ...
                foreach ($arrTopThree as $topShare) {
                    ...
                    echo '<tr>';
                    echo '<td>'.$intCount.'</td>';
                    echo '<td><a href="'.$topShare['url'].'">'.urldecode($topShare['title']).'</a></td>';
                    ...
                    $arrPostData['title'] = $topShare['title'];
            ...
            echo '<td>'.$latestShare['datetime'].'</td>';
                ...
                if ($latestShare['geoip'] == '' && $latestShare['ip'] != '127.0.0.1') {
                    ...
                    if ($ssbpLicense_key && $ssbpLicense_key != '') {
                        ...
                        // call ssb api
                        $jsonGeoIP = wp_remote_post('https://simplesharebuttons.com/api/', array(
                                                                    'method' => 'POST',
                                                                    'timeout' => 10,
                                                                    'body' => array(
